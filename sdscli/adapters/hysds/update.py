...
def update_mozart(conf, ndeps=False, config_only=False, comp='mozart'):
    ...
    with tqdm(total=num_updates) as bar:  # progress bar
        ...
        for script in ['log_instance_stats.sh', 'ingest_dataset.sh', 'process_events.sh', 'watchdog_job_timeouts.sh', 
            ...
            execute(fab.run, f'cp {wrapper_src_path} ~/mozart/bin/{script}', roles=[comp])
        ...
        execute(fab.mkdir, '~/mozart/ops/mozart', None, None, roles=[comp])
...
def update_metrics(conf, ndeps=False, config_only=False, comp='metrics'):
    ...
    with tqdm(total=num_updates) as bar:  # progress bar
        ...
        wrapper_src = execute(fab.get_package_resource_path, 'hysds', 'scripts/log_instance_stats.sh',
                              remote=True, resource_type='file', roles=[comp])
        wrapper_src_path = list(wrapper_src.values())[0]
        execute(fab.run, f'cp {wrapper_src_path} ~/metrics/bin/log_instance_stats.sh', roles=[comp])
...
def update_grq(conf, ndeps=False, config_only=False, comp='grq'):
    ...
    with tqdm(total=num_updates) as bar:  # progress bar
        ...
        wrapper_src = execute(fab.get_package_resource_path, 'hysds', 'scripts/log_instance_stats.sh',
                              remote=True, resource_type='file', roles=[comp])
        wrapper_src_path = list(wrapper_src.values())[0]
        execute(fab.run, f'cp {wrapper_src_path} ~/sciflo/bin/log_instance_stats.sh', roles=[comp])
        ...
        execute(fab.mkdir, '~/sciflo/ops/grq2', None, None, roles=[comp])
        execute(fab.mkdir, '~/sciflo/ops/pele', None, None, roles=[comp])
